  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (isTimestampMarker) {
        ...
        // If stuck > STALE_MS: check if order already saved → delete row, else revert from backup
        const uniqueCode = rest.toUpperCase();
        const orderAlreadySaved = orderCodes.has(uniqueCode);
...
async function getNextAvailableAccount(sheetName, uniqueCode) {
  ...
  // IMPORTANT: if deliveredSet fetch fails, we must abort (return null) — never deliver with empty set
  ...
  try {
    ...
    console.error('[sheets] ABORT getNextAvailableAccount — could not fetch stock or deliveredSet:', fetchErr.message);
    return null; // Safer to report out-of-stock than to risk duplicate delivery
  }
...
async function isAccountAlreadyDelivered(accountEmail, accountPassword) {
  ...
  try {
    ...
    const rows = res.data.values || [];
    // Normalize: lowercase + remove spaces around colon
    ...
    // FAIL SAFE: if we can't verify, assume it IS delivered to prevent duplicates
    console.warn('[sheets] isAccountAlreadyDelivered check FAILED — blocking delivery to be safe:', e.message);
    return true;
  }
}
