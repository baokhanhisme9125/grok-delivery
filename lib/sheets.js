...
async function getDeliveredAccountSet(sheets, ordersSheetName) {
  // NOTE: If this throws, caller must abort — do NOT catch and continue with empty set.
...
async function cleanupClaimedRows(sheets, sheetTab, sheetId) {
  try {
    const [stockRes, ordersRes] = await Promise.all([
      ...
      sheets.spreadsheets.values.get({
        spreadsheetId: SPREADSHEET_ID,
        range: `'${ORDERS_SHEET}'!A:C`,   // Read A:C to check if order is completed (C) or pending (C empty)
      }).catch(() => ({ data: { values: [] } })),
    ]);

    const rows = stockRes.data.values || [];
    // Only include COMPLETED orders (column C has account email:password)
    // Pending orders (column C empty) should NOT cause stock rows to be deleted
    const completedOrderCodes = new Set(
      (ordersRes.data.values || [])
        .filter(r => (r[2] || '').includes(':'))  // column C has account
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (isTimestampMarker) {
        ...
        if (now - ts > STALE_MS) {
          if (backup) {
            console.warn(`[sheets] Reverting stale timestamp CLAIMED row ${i + 1} from backup`);
            try {
            ...
            console.warn(`[sheets] Stale timestamp CLAIMED row ${i + 1} has NO backup — account may be lost`);
          }
        }
      } else {
        // UniqueCode-style marker (e.g. CLAIMED:A05D736647B54AB1)
        // These should have been deleted by verify.js after delivery.
        // Only delete stock row if order is COMPLETED (column C has account).
        // If order is pending (column C empty), delivery failed → revert claim to stock.
        const uniqueCode = rest.toUpperCase();
        const orderAlreadySaved = completedOrderCodes.has(uniqueCode);

        if (orderAlreadySaved) {
          // Order was saved but row wasn't deleted → delete it now
          ...
          try {
            ...
            // rows shifted — adjust index
            rows.splice(i, 1);
            i--;
          } catch (e) { console.warn(`[sheets] Failed to delete stuck row ${i + 1}:`, e.message); }
        } else {
          // No order saved — this is a stuck claim (crash, race) → revert from backup
          // Only revert if stuck more than STALE_MS (normal in-flight claims need time)
          // We use the backup timestamp heuristic: if no info, use 10 min to be safe
          if (backup) {
            console.warn(`[sheets] Reverting stuck CLAIMED:${uniqueCode} row ${i + 1} from backup (no order found)`);
            try {
              await sheets.spreadsheets.values.update({
                spreadsheetId: SPREADSHEET_ID,
                range: `'${sheetTab}'!A${i + 1}:B${i + 1}`,
            ...
            console.warn(`[sheets] Stuck CLAIMED:${uniqueCode} row ${i + 1} has NO backup — account may be lost`);
          }
        }
      }
    }
...
async function getNextAvailableAccount(sheetName, uniqueCode) {
  ...
  // Run cleanup of stale CLAIMED rows in background — only 20% of requests to save API quota
  if (Math.random() < 0.20) {
    cleanupClaimedRows(sheets, sheetTab, sheetId).catch(() => {});
  }

  // Fetch stock (A:B for backup) + delivered accounts
  // Retry once on failure — transient Sheets API errors should not block all deliveries
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    // Duplicate guard — account already delivered to another buyer
    ...
    if (deliveredSet.has(normalized)) {
      ...
      // Delete this stale row so it doesn't clog the stock sheet
      try {
        await sheets.spreadsheets.batchUpdate({
          spreadsheetId: SPREADSHEET_ID,
          requestBody: {
        ...
        rows.splice(i, 1); // adjust local array
        i--;               // re-check same index after splice
      } catch (e) {
        console.warn(`[sheets] Could not delete stale delivered row ${i + 1}:`, e.message);
      }
      continue;
    }
    ...
    if (verifyCell === claimMark) {
      return { email, password, claimMark, _sheetTab: sheetTab, _sheetId: sheetId, _deliveredSet: deliveredSet };
    }
...
/**
 * REVERT a CLAIMED row back to its original account value using Column B backup.
 * Use this when delivery is BLOCKED (duplicate detected, API error, race lost).
 * DO NOT use deleteAccountRow for blocked deliveries — that permanently loses the account.
 */
...
async function isAccountAlreadyDelivered(accountEmail, accountPassword) {
  ...
  // Try up to 2 times for transient API errors
  for (let attempt = 1; attempt <= 2; attempt++) {
    try {
      const sheets = await getSheetsClient();
      const res = await sheets.spreadsheets.values.get({
  ...
  // After retries: return false (allow delivery) — account loss is worse than rare duplicate.
  // Primary protection is deliveredSet in getNextAvailableAccount.
 ...
 * Used to prevent double-delivery when buyer submits a different unique code for the same order.
 */
