...
async function getNextAvailableAccount(sheetName, uniqueCode) {
  ...
  // ── Guard: if this uniqueCode is ALREADY claimed in the sheet, ──────────
  // another concurrent process (webhook/verify race) is handling it.
  // Wait for it to finish, then return null so caller falls through to
  // the idempotency check (findOrderByCode) which will find the saved order.
  if (uniqueCode) {
    const alreadyClaimed = rows.some(r => (r[0] || '').trim() === `CLAIMED:${uniqueCode}`);
    if (alreadyClaimed) {
      console.log(`[sheets] CLAIMED:${uniqueCode} already exists — waiting for other process to finish...`);
      await new Promise(r => setTimeout(r, 800));
      return null; // Caller should retry via findOrderByCode idempotency check
    }
  }
