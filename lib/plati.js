...
async function verifyUniqueCode(uniqueCode) {
   ...
   * Only allow unique codes from specific Grok product IDs.
   * Set ALLOWED_GOODS_IDS env var as comma-separated Digiseller product IDs.
   */
  const allowedIds = (process.env.ALLOWED_GOODS_IDS || '').split(',').map(s => s.trim()).filter(Boolean);
  const goodsId = String(ucData.id_goods || '');
  if (allowedIds.length > 0 && !allowedIds.includes(goodsId)) {
