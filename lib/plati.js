...
async function verifyUniqueCode(uniqueCode) {
  ...
  /* ── Product whitelist check (HARDCODED + ENV) ──────────────────────
   * Hardcoded fallback: 5126799 (Grok Account on Digiseller)
   * ALLOWED_GOODS_IDS env var can add more IDs if needed.
   */
  const HARDCODED_IDS = ['5126799'];
  const envIds = (process.env.ALLOWED_GOODS_IDS || '').split(',').map(s => s.trim()).filter(Boolean);
  const allowedIds = [...new Set([...HARDCODED_IDS, ...envIds])];
  const goodsId = String(ucData.id_goods || '');
  console.log(`[plati] code=${uniqueCode} goods_id=${goodsId} buyer=${ucData.email || 'unknown'} allowed=[${allowedIds.join(',')}]`);
  if (!allowedIds.includes(goodsId)) {
    console.warn(`[plati] BLOCKED: code=${uniqueCode} goods_id=${goodsId} buyer=${ucData.email} — NOT in allowed list`);
  ...
  return {
    ...
    datePay:     String(ucData.date_pay || ucData.date_confirm || ucData.date || ''),
    productType: 'grok',
